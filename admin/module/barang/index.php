 		...
 		<div class="row">
 			<div class="col-lg-12 main-chart">
 				...
 				<?php
								...
								// Ambil nilai stok minimal dari database
				$query = $config->prepare("SELECT minimal_stok FROM konfigurasi_stok LIMIT 1");
				$query->execute();
				$data = $query->fetch(PDO::FETCH_ASSOC);
				$minimal_stok = $data['minimal_stok'];

				$sql = "SELECT * FROM barang WHERE stok <= :minimal_stok";
				$row = $config->prepare($sql);
				$row->bindParam(':minimal_stok', $minimal_stok, PDO::PARAM_INT);
				$row->execute();
				$r = $row->rowCount();
 				...
 				<?php
						echo "
								<div class='alert alert-warning'>
									<span class='glyphicon glyphicon-info-sign'></span> Ada <span style='color:red'>$r</span> barang yang Stok tersisa sudah kurang dari $minimal_stok items. silahkan pesan lagi !!
                                    ...
                                    <td>Merk Barang</td>
                                        <td>
											<select name="merk" class="form-control" required>
                                                <option value="<?php echo $hasil['merk'];?>"><?php echo $hasil['merk'];?></option>
													<option value="#">Pilih Merk</option>
													<option value="mely">Mely</option>
													<option value="xuping">Xuping</option>
													<option value="yaxiya">Yaxiya</option>
											</select>
										</td>
								<tr>
 										<tr>
                                                <td>Harga Beli</td>
                                                <td><input type="number" required class="form-control" name="beli" value="10000"></td>
                                            </tr>
                                            <tr>
                                                <td>Harga Jual</td>
                                                <td><input type="number" required class="form-control" name="jual" value="20000"></td>
                                            </tr>
                                            <tr>
                                                <td>Satuan Barang</td>
                                                <td>
                                                    <input type="hidden" name="satuan" value="Gram">
                                                    Gram
                                                </td>
                                            </tr>
 										<tr>
